function [Data,gnd]=DataCreation()
clear;
clc;

Data=zeros(1,10304);
gnd=zeros(1,1);
listfolders=dir('C:');%#Enter your Project Data Folder Location
dirIndex = [listfolders.isdir];
 fileList = {listfolders(~dirIndex).name}';  %'# Get a list of the files
  if ~isempty(fileList)
    fileList = cellfun(@(x) fullfile(listfolders,xl),...  %# Prepend path to files
    ...
    for iDir = 1:40
    subfolder=fullfile('C:',sortedCellArray{iDir}); #Enter the Data Folder Location
